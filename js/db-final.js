...
auth.onAuthStateChanged(user => {
    if (!user) {
        ...
        let renderEventRegistration = (item, i) => {
            ...
            el.innerHTML = `<h1 style="text-align: center;background: #fff;border: 1px solid #b9b9b9;border-radius: 20px;padding: 1em;">${item.data().eventName}</h1><h3 style="color: #ff8e8e;font-weight:600;">${item.data().startAt} - ${item.data().endAt}</h3><h4 style="font-weight:500;">${item.data().eventDesc}</h4><h4 style="font-weight:600;">Under the supervison -  ${item.data().coordinatedBy}</h4>`
                ...
                if (item.data().category === 'photo') {
                    db.collection('event').doc(item.id).collection('liked').onSnapshot(r => {
                        if (r.size === 0) {
                            ...
                            el.innerHTML = '<div class="profile"><div class="profile-section"><img src="' + result.profilePic + '" alt="" class="profile-pic"><div class="displayName">' + displayName + '</div></div><i class="im im-menu-dot-v"></i></div><img src="' + item.data().eventPicUrl + '" alt="" class="post-pic"><div class="desc"><div style="padding: 1em 0em; font-weight: 700;">' + r.size + ' likes</div><div class="">' + item.data().caption + '</div><div class="reaction"><i id="' + item.data().photoInterestId + '" class="im im-star-o"></i><i class="im im-data"></i><i class="im im-menu-dot-h"></i></div>'
                    ...
                    el.innerHTML = '<div class="profile"><div class="profile-section"><img src="' + result.profilePic + '" alt="" class="profile-pic"><div class="displayName">' + displayName + '</div></div><i class="im im-menu-dot-v"></i></div><img src="' + item.data().eventPicUrl + '" alt="" class="post-pic"><div class="desc">' + item.data().eventDesc + '</div><div class="reaction"><i class="im im-rocket"></i><i id="' + item.data().eventRegisterId + '" class="im im-barcode"></i><i class="im im-menu-dot-h"></i></div>'
                }
        ...
        let displayUserList = value => {
            ...
            firebase.firestore().collection('users').doc(value.data().uid).onSnapshot(rt => {
                if (rt.data()) {
                    ...
                    document.getElementById(finalStr).style.borderRadius = '4px 0px 0px 4px'
                } else {
                    //
                    ...
                    d.forEach(snap => {
                        if (!snap.doc.metadata.hasPendingWrites) {
                            ...
                            if (snap.doc.data().sendTo === obj.name) {
                                ...
                                li.innerHTML = `<div class="msg" style="border-radius: 13px 0px 13px 13px;">${snap.doc.data().text}</div>`
                            } else {
                                li.classList.add('receiver')
                                li.innerHTML = `<div class="msg" style="background: #283996; border-radius: 0px 13px 13px 13px;">${snap.doc.data().text}</div><div class="stopper"></div>`
                            }
                ...
                snapshot.forEach(data => {
                    ...
                    if (!d.metadata.hasPendingWrites) {
                        if (d.data().uid === getUser()) {
                            ...
                            document.querySelector('#profilePic').src = d.data().profilePic || "https://firebasestorage.googleapis.com/v0/b/chat-294407.appspot.com/o/events%2Fprofile.jpg?alt=media&token=84fe27e6-82e7-4599-baca-d1cd92cb9a57"
                            document.querySelector('#userDisplayName').innerText = d.data().displayName || d.data().email
                        } else {
                            ...
                            document.getElementById(finalStr).addEventListener('click', e => {
                                ...
                                toggleUser.innerHTML = `<i class="im im-angle-right button"></i><div style="display: inline;font-weight: 700;font-family: sans-serif;font-size: 20px;">${v}</div>`
                                toggleUser.style.fontFamily = "monospace"
                                obj.name = d.data().uid
